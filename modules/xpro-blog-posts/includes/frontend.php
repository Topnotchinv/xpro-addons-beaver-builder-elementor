...
$xpro_args = $module->get_xpro_args();
$args      = $module->render_args();

$xpro_args = apply_filters( 'xpro_blog_posts_query_args', $args, $settings );

$module->set_xpro_args( $xpro_args );
...
$col = ( 'carousel' !== $settings->is_carousel ) ? ( ( 'feed' === $settings->is_carousel ) ? 1 : $settings->post_per_grid ) : $settings->post_per_grid;

$col = ( 'carousel' === $settings->is_carousel ) ? $settings->post_per_grid : ( ( 'feed' === $settings->is_carousel ) ? 1 : $settings->post_per_grid );
...
?>
<div class="xpro-module-content xpro-blog-posts <?php echo wp_kses_post( ( 'carousel' === $settings->is_carousel ) ? 'xpro-blog-posts-carousel' : ( ( 'grid' === $settings->is_carousel ) ? 'xpro-blog-posts-grid' : '' ) ); ?> xpro-post-grid-<?php echo esc_attr( $col ); ?> <?php echo ( 'masonary' === $settings->is_carousel ) ? ' xpro-blog-posts-masonary ' : ''; ?>">
	<?php
	$class = '';
	...
	for ( $i = 0; $i < $condition; $i++ ) {
					...
					if ( -1 !== (int) $settings->$cat ) {
						...
						if ( count( $category_detail ) > 0 ) {
							foreach ( $category_detail as $cat_details ) {
								$class .= ' xpro-masonary-cat-' . $cat_details->term_id . ' ';
							}
						}
					}
				}
		...
		$left_hide_class = ( '' === $left_featured_image_content && '' === $right_featured_image_content ) ? 'xpro-empty-img' : '';

		do_action( 'xpro_blog_posts_before_post', $the_query->posts[ $i ]->ID, $settings );
		?>
	<div class="xpro-blog-posts-col-<?php echo esc_attr( $col ); ?> xpro-post-wrapper <?php echo ( 'masonary' === $settings->is_carousel ) ? ' xpro-blog-posts-masonary-item-' . esc_attr( $module->node ) . ' ' : ''; ?> <?php echo ( 'masonary' === $settings->is_carousel || 'grid' === $settings->is_carousel ) ? esc_attr( $class ) : ''; ?> <?php echo ( 'grid' === $settings->is_carousel ) ? ' xpro-blog-posts-grid-item-' . esc_attr( $module->node ) . ' ' : ''; ?>">
		<div class="xpro-blog-posts-shadow clearfix">

			<div class="xpro-blog-post-inner-wrap <?php echo 'xpro-thumbnail-position-' . esc_attr( $settings->blog_image_position ); ?> <?php echo ( 'img,title,meta,content,cta' !== $settings->layout_sort_order ) ? 'xpro-blog-reordered' : ''; ?> <?php echo esc_attr( $left_hide_class ); ?>">
			<?php
			if ( isset( $settings->post_layout ) && 'custom' === $settings->post_layout && defined( 'FL_THEME_BUILDER_DIR' ) ) {
				include XPRO_ADDONS_FOR_BB_DIR . 'includes/' . $module->slug . '-frontend.php';
			} else {
		...
		<?php
		do_action( 'xpro_blog_posts_after_post', $the_query->posts[ $i ]->ID, $settings );
	}
...
if ( 'carousel' !== $settings->is_carousel && 'yes' === $show_pagination ) {
	...
	if ( 'masonary' === $settings->is_carousel ) {
		if ( true === $do_pagination ) {
			?>
		<div class="xpro-blogs-pagination"
			<?php
